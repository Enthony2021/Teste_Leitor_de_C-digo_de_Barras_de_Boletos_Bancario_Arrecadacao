...
class Arrecadacao {
  ...
  constructor(code: string) {
    this.code = code;
    this.barCode = `${this.code.slice(0, 11)}${this.code.slice(
      12,
      23
    )}${this.code.slice(24, 35)}${this.code.slice(36, 47)}`;
  ...
  public validacaoArrecadacaoTamanho(): boolean {
    return parseInt(this.bloco1[0]) == 8 && this.code.length === 48
      ? true
      : false;
  }
  ...
  private validacaoDV(): boolean {
    ...
    if (this.idValorReferencia === 6 || this.idValorReferencia === 7) {
      ...
      for (let i = 0; i < blocoDACInv.length; i++) {
        i % 2 === 0
          ? (produto = parseInt(blocoDACInv[i]) * 2)
          : (produto = parseInt(blocoDACInv[i]) * 1);
  ...
  // Valor (posição 5-15)
  private getValor(): string | null {
    // Valor a ser cobrado
    if (this.idValorReferencia === 6 || this.idValorReferencia === 8) {
      const valor: string = (parseInt(this.barCode.slice(4, 15)) / 100).toFixed(2)
  ...
  // DatadeVencimento
  private getDataDeVencimento(): null {
  ...
  validacaoArrecadacao(): Result | boolean {
    if (
      this.validacaoArrecadacaoTamanho() === true &&
  ...
  testeBoleto() {
    const results = {
      ...
      code: this.barCode, // String
      amount: this.getValor(), // String ou null
      expirationDate: this.getDataDeVencimento(), // null
    };
